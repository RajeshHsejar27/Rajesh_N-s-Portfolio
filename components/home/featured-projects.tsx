...
// export function FeaturedProjects() {
//   // --- This line filters your projects to show only those marked as featured ---
...
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
//           {/* --- Updated from PROJECTS.map to featured.map --- */}
...
// --- Sub-component for Featured Individual Cards ---
function FeaturedProjectCard({ project, index, handleOpenModal }: { 
  project: any; 
  index: number; 
  handleOpenModal: (status?: string) => void 
}) {
  ...
  return (
    ...
    >
      <Card className="h-full flex flex-col hover:shadow-lg transition-shadow duration-300">
        ...
        <CardHeader>
          <CardTitle className="text-xl">{project.title}</CardTitle>
          <CardDescription className={cn("transition-all duration-300", !isExpanded && "line-clamp-3")}>
            {project.description}
          </CardDescription>
          <button 
            onClick={() => setIsExpanded(!isExpanded)}
            className="text-xs font-bold text-blue-600 mt-2 text-left w-fit hover:underline"
          >
